...
'''
# Uncomment this section if you want python to show you a plot of your track
# Very slow (1-4min), thus disabled by default.
...
class Window:
    def __init__(self, master):
        ...
        # GPX Path Button
        self.b_gpxUp = Button(text="Upload GPX-Track",bg='yellow',command=lambda:self.trackUpload('main'))
        self.b_gpxUp.grid(row=1,column=1,padx=0,pady=(10,10),sticky='w')

        # Tracking Mistake Detector Button
        self.b_trackMist = Button(text='Show Tracking Mistakes',command=self.trackMistakes)
        ...
        self.b_gpxUpSnipp = Button(text='Upload GPX fragment',bg='yellow', command=lambda:self.trackUpload('snip'))
        ...
        self.e_dist.bind("<FocusIn>",self.clickOn)

        # Confirmation button for speed submission
        self.b_OK = Button(text='OK',command=self.readDistance)
    ...
    def trackUpload(self,fileType):
        ...
        self.gpx[fileType]['path'] = filedialog.askopenfilename(parent=root,title='Choose a file')
        ...
        self.b_showMap = Button(self.win_succ,text='Show GPX on map (very slow)',
                command=lambda:self.make_map(fileType,'plain'))
        self.b_showMap.grid(row=2,column=1,padx=20,pady=20)
        self.win_succ_mess = Label(self.win_succ,text='Upload and parsing of GPS successful')
    ...
    def trackMistakes(self):
        ...
        try:
            ...
            self.messageWindow('File Error', """Please upload a valid GPX track (yellow button) before"""+
                """ you try to detect the tracking mistakes.""", 250, 150)
            return None
        ...
        text = ['#','From (Long,Lat)', 'To (Long,Lat)', 'Distance (m)','View "#" on GoogleMaps']
        weights = [1,2,2,2,3]
        for k in range(len(text)):
            self.win_links.columnconfigure(k,weight=weights[k])
            label = Label(self.win_links,text=text[k]).grid(row=0,column=k, pady=10,sticky='w')

        if len(self.gpx['main']['trackHoles'])==0:
            mess = Label(self.win_links,text='Great! No error has been found!').grid(row=1, column=1,pady=10,sticky='w')
        ...
        for run,errInd in enumerate(self.gpx['main']['trackHoles']):
            ...
            num = Label(self.win_links,text=str(run+1)).grid(row=run+1,column=0,pady=10,sticky='w')

            # Display start end
            start = Label(self.win_links,text=(startLat+' , '+startLong)).grid(row=run+1,column=1,pady=10,sticky='w')
            stop = Label(self.win_links,text=(endLat+' , '+endLong)).grid(row=run+1,column=2,pady=10,sticky='w')

            dist = Label(self.win_links,text=str(round(self.gpx['main']['trackHoleSizes']
                [run],1))).grid(row=run+1,column=3,pady=10,sticky='w')

            self.links.append(self.GM_start+startLat+','+startLong+'/'+endLat+','+endLong+self.GM_end)
        
        # For convenience: Put link for mapstogpx
        mapstogpx = Button(self.win_links,text='GoogleMaps to GPX',fg='blue',cursor='hand2',
        ...
        missStartString =self.GM_start+'/'+str(self.gpx['main']['plain'][0,0])+','+str(self.gpx['main']['plain'][0,1])+self.GM_end
        missStart = Button(self.win_links,text='I miss the start of my ride',fg='blue',cursor='hand2',
            command=lambda:webbrowser.open_new(missStartString))
        missStart.grid(row=run+3,column=2,pady=10,sticky='w')
        missEndString =self.GM_start+str(self.gpx['main']['plain'][-1,0])+','+str(self.gpx['main']['plain'][-1,1])+self.GM_end
        missEnd = Button(self.win_links,text='I miss the end of my ride',fg='blue',cursor='hand2',
        ...
        popupMenu = OptionMenu(self.win_links, tkvar, *list(range(1,len(self.links)+1)))
        popupMenu.grid(row=1,column=4,sticky='w')
 
        # Confirmation button
        ok_but = Button(self.win_links,text='GO!',fg='blue',cursor='hand2',
    ...
    def extractParam(self,fileType):
        ...
        # Save name of file
        if fileType=='main':
            self.filename = self.gpx['main']['parsed'].tracks[0].name

        # Allocate space
        self.gpx[fileType]['plain'] = np.zeros((len(self.gpx[fileType]['parsed'].tracks[0].segments[0].points),4))
            ...
            for segment in track.segments:
                for ind,point in enumerate(segment.points):
                    ...
                    if ind > 0 and fileType=='main':
                        timeDiff = (point.time-segment.points[ind-1].time).total_seconds()
                        self.gpx[fileType]['plain'][ind,3] = timeDiff
                        # Save indices of tracking errors
                        if timeDiff > self.timeThreshold:
                            euclid = gpxpy.geo.haversine_distance(point.longitude,point.latitude,
                                segment.points[ind-1].longitude,segment.points[ind-1].latitude)
        ...
        if fileType=='main':
            self.gpx[fileType]['startTime'] = self.gpx[fileType]['parsed'].tracks[0].segments[0].points[0].time
            self.gpx[fileType]['finishTime'] = self.gpx[fileType]['parsed'].tracks[0].segments[0].points[-1].time
    ...
    def make_map(self,fileType, mapType):
        '''
        This function plots the ride on a cartographic map.
        It receveis the fileType (main, snip) and the mapType(plain,analysed).

        At this point, the function is VERY slow (program hangs for 1-4 min depending on your machine and the 
            length of your ride). Thus, mapsOff is True by default, such that the map is not printed. See top 
            of file how to enable map plotting.    
        '''

        if mapsOff:
            # Information window that this function is disabled.
            self.win_off = Toplevel(self.master)
            self.win_off_frame = Frame(self.win_off)
            self.win_off.minsize(width=250,height=100)
            self.win_off.wm_title('Rejection')
            # Option in confirmation window to show the GPX Track
            self.b_showMap.grid(row=2,column=1,padx=20,pady=20)
            self.win_off_mess = Label(self.win_off,text='This function is disabled by default due to slow processing.\n'+
                ' Please check out the code to enable it.')
            self.win_off_mess.grid(row=1,column=1,padx=20,pady=20)

        else:   # Okay, you really want the map... Get you a coffee and wait :)

            request = cimgt.OSM()
            # Define borders
            data = self.gpx[fileType]
            longOff = (data['maxLong']-data['minLong'])*0.1
            latOff = (data['maxLat']-data['minLat'])*0.1
            mapBorders = [data['minLong']-longOff,data['maxLong']+longOff,data['minLat']-latOff, data['maxLat']+latOff]

             # Compute Map
            fig, ax = plt.subplots(figsize=(9, 13),
                                   subplot_kw=dict(projection=request.crs))
            ax.set_extent(mapBorders)
            img = ax.add_image(request, 14)
            fig.canvas.set_window_title('GPX Track')

            # Sort datapoints    
            Longs_Err = np.concatenate((data['plain'][data['trackHoles'],1], 
                data['plain'][[x-1 for x in data['trackHoles']],1])).tolist()
            Lats_Err = np.concatenate((data['plain'][data['trackHoles'],0], 
                data['plain'][[x-1 for x in data['trackHoles']],0])).tolist()
            Longs = np.concatenate((np.delete(data['plain'][:,1],data['trackHoles']), 
                np.delete(data['plain'][:,1],[x-1 for x in data['trackHoles']]))).tolist()
            Lats = np.concatenate((np.delete(data['plain'][:,0],data['trackHoles']), 
                np.delete(data['plain'][:,0],[x-1 for x in data['trackHoles']]))).tolist()

            LongsShort = [l for ind,l in enumerate(Longs) if ind%self.resolution == 0]
            LatsShort  = [l for ind,l in enumerate(Lats) if ind%self.resolution == 0]

            # Plot datapoints
            plt.plot([LongsShort], [LatsShort],color='blue', marker='o',markersize=2, transform=ccrs.PlateCarree())
            errorCol = 'blue' if mapType=='plain' else 'red'
            errorSize = 10 if mapType=='analysed' else 2
            plt.plot([Longs_Err], [Lats_Err],color=errorCol, marker='o',markersize=errorSize, transform=ccrs.PlateCarree())
            plt.show()
            return None
    ...
    def Merge(self):
        ...
        try:
            ...
            self.messageWindow('Instruction',"""Please insert a valid distance in m and press the "OK" button.""",
                200,100)
            return None
        if self.dist < 0:
            self.messageWindow('Instruction',"""Please insert a valid distance in m and press the "OK" button.""",
        ...
        try:
            ...
            self.messageWindow('File Error', """Please upload a valid GPX main track before you try to repair it.""",
                200,100)
            return None
        ...
        try: 
            ...
            self.messageWindow('FileError', """Please upload a valid GPX snippet which the programme can use to"""+
                """ repair the main file.""",200,100)
            return None
        ...
        np.array([self.gpx['snip']['parsed'].tracks[0].segments[0].points[0].latitude, self.gpx['snip']['parsed'].tracks[0].segments[0].points[0].longitude])
        
        # Find the right position of the snippet.
        dists = [gpxpy.geo.haversine_distance(*np.concatenate((dataOld[ind-1,0:2],var)).tolist()) for ind in self.gpx['main']['trackHoles']]
        ...
        dists.append(gpxpy.geo.haversine_distance(*np.concatenate((dataOld[0,0:2],np.array([dataNew[-1].latitude, dataNew[-1].longitude])))))
        dists.append(gpxpy.geo.haversine_distance(*np.concatenate((dataOld[-1,0:2],var))))

        if min(dists) > self.distThreshold:
            self.messageWindow('Data Error', """This GPX snippet does not match to file you try to repair.""",200,100)
            return None
        ...
        while indO < thresh:
            gpx_segment.points.append(gpxpy.gpx.GPXTrackPoint(dataOld[indO,0],dataOld[indO,1],
                elevation=dataOld[indO,2], time=self.gpx['main']['parsed'].tracks[0].segments[0].points[indO].time))
            indO += 1

        
        # Compute cumulative time needed for the snippet (in seconds)
        if thresh != len(dataOld) and thresh != 0: # Regular case
            cum_Time = (self.gpx['main']['parsed'].tracks[0].segments[0].points[thresh+1].time
                -gpx_segment.points[-1].time).total_seconds()
        
            # Compute cumulative (pointwise) distance of GPX track and compare to GoogleMaps
            dataNew.append(gpx_segment.points[-1]) # To be able to compute distance for first point
            cumDist = 0
            for ind,point in enumerate(dataNew):
                cumDist += gpxpy.geo.haversine_distance(dataNew[ind-1].latitude,dataNew[ind-1].longitude,
            ...
            self.messageWindow('Error intensity ', """You inserted that the GoogleMaps distance is """+
                str(round(self.dist))+"""m for this snippet, while the cumulative distance of GPX points is """+
                str(round(cumDist))+"""m. Therefore the overestimation is """+str(round(self.error,3))+
                '''%. The algorithm smoothes this out. The average speed was '''+str(round(self.speed*3.6,2))+
                    """km/h.""",400,200)
            ...
            if thresh == 0: # Workaround if we missed the starrt
                t = self.gpx['main']['parsed'].tracks[0].segments[0].points[0].time - datetime.timedelta(0,self.dist/self.speed)
                gpx_segment.points.append(gpxpy.gpx.GPXTrackPoint(dataNew[0].latitude,dataNew[0].longitude,
                    elevation=dataNew[0].elevation,time=t))
        ...
        while indN < len(dataNew)-1:
            ...
            stepDist = gpxpy.geo.haversine_distance(gpx_segment.points[-1].latitude, gpx_segment.points[-1].longitude,
                dataNew[indN].latitude,dataNew[indN].longitude)

            # Skip data point if not futher than 10m away from last 
            if stepDist > 10:
                stepTime = stepDist/self.speed
                prevTime = gpx_segment.points[-1].time
                gpx_segment.points.append(gpxpy.gpx.GPXTrackPoint(dataNew[indN].latitude,dataNew[indN].longitude,
                    elevation=dataNew[indN].elevation,time=prevTime+datetime.timedelta(0,stepTime)))
            indN += 1
        ...
        self.messageWindow('Success!','Congratulations! The track is repaired and saved in the '+
            str(directory)+' Folder.',250,150)
